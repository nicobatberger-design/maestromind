...
export default function OutilsPage() {
  ...
  "── ÉLECTRICITÉ ──", "Tableau électrique", "Prises + interrupteurs", "Éclairage LED", "VMC simple flux", "VMC double flux",
        ...
        {toolTab === "planning" && <div>
          <div style={{ fontSize: 9, color: "#C9A84C", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>📅 Planification de chantier</div>
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 9, color: "rgba(240,237,230,0.45)", marginBottom: 4, textTransform: "capitalize", letterSpacing: 0.5 }}>Type de projet</div>
            <select style={s.inp} value={planningType} onChange={e => setPlanningType(e.target.value)}>{["Rénovation appartement", "Rénovation maison", "Construction neuve", "Extension", "Surélévation"].map(t => <option key={t}>{t}</option>)}</select>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
            <div><div style={{ fontSize: 9, color: "rgba(240,237,230,0.45)", marginBottom: 4, textTransform: "capitalize", letterSpacing: 0.5 }}>Surface m²</div><input style={s.inp} type="number" inputMode="decimal" value={planningSurface} onChange={e => setPlanningSurface(e.target.value)} placeholder="50" /></div>
            <div><div style={{ fontSize: 9, color: "rgba(240,237,230,0.45)", marginBottom: 4, textTransform: "capitalize", letterSpacing: 0.5 }}>Code postal (météo)</div><input style={s.inp} type="text" inputMode="numeric" maxLength={5} value={planningCP} onChange={e => setPlanningCP(e.target.value.replace(/\D/g, "").slice(0, 5))} placeholder="75001" /></div>
          </div>
          <button style={planningLoading ? { ...s.greenBtn, opacity: 0.5 } : s.greenBtn} onClick={planifierChantier} disabled={planningLoading}>{planningLoading ? "Planification en cours..." : "📅 Générer le planning"}</button>
          {planningResult && <div style={{ marginTop: 14, maxHeight: 420, overflowY: "auto", WebkitOverflowScrolling: "touch", paddingRight: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}><div style={{ fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 800 }}>{planningResult.duree_totale}</div><div style={{ fontSize: 11, color: "rgba(240,237,230,0.5)" }}>· {planningType} · {planningSurface} m²</div></div>
