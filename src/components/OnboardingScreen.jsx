...
export default function OnboardingScreen() {
  ...
  return (
    <>
      ...
      <div className="grain-overlay" style={{ display: "flex", flexDirection: "column", height: "100vh", maxWidth: 430, margin: "0 auto", background: "#06080D", color: "#F0EDE6", fontFamily: "'DM Sans',sans-serif", alignItems: "center", justifyContent: "flex-start", paddingTop: "12vh", padding: "12vh 32px 0 32px", position: "relative", overflow: "hidden" }}>
        {/* Ambient glow orbs */}
        <div style={{ position: "absolute", top: -100, left: "50%", transform: "translateX(-50%)", width: 360, height: 360, borderRadius: "50%", background: "radial-gradient(circle,rgba(201,168,76,0.18) 0%,rgba(201,168,76,0.04) 40%,transparent 70%)", pointerEvents: "none", animation: "ambientGlow 5s ease-in-out infinite" }} />
        <div style={{ position: "absolute", top: "40%", right: -60, width: 200, height: 200, borderRadius: "50%", background: "radial-gradient(circle,rgba(82,144,224,0.06) 0%,transparent 70%)", pointerEvents: "none", animation: "orbFloat 8s ease-in-out infinite 1s" }} />
        <div style={{ position: "absolute", bottom: "20%", left: -40, width: 160, height: 160, borderRadius: "50%", background: "radial-gradient(circle,rgba(82,195,122,0.04) 0%,transparent 70%)", pointerEvents: "none", animation: "orbFloat 10s ease-in-out infinite 2s" }} />
        ...
        <button className="gold-btn" onClick={() => { if (onboardingStep < steps.length - 1) { setOnboardingStep(prev => prev + 1); } else { localStorage.setItem("bl_onboarded", "1"); localStorage.setItem("bl_user_type", userType); setOnboardingDone(true); } }} style={{ width: "100%", background: "linear-gradient(135deg,#EDD060,#C9A84C,#8A6820)", border: "none", borderRadius: 16, padding: "16px", fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 800, color: "#06080D", cursor: "pointer", boxShadow: "0 4px 28px rgba(201,168,76,0.4), 0 0 60px rgba(201,168,76,0.1)", letterSpacing: "0.5px" }}>
