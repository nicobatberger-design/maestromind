      ...
      <div style={s.wrap}>
        ...
        {projets.map(p => <div key={p.id} style={{ ...s.card, marginBottom: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginTop: 10 }}>
            <button onClick={() => ouvrirProjetChat(p)} style={{ background: "rgba(201,168,76,0.08)", border: "0.5px solid rgba(201,168,76,0.25)", borderRadius: 10, padding: "7px 4px", fontSize: 9, fontWeight: 700, color: "#C9A84C", cursor: "pointer" }}>{"\u{1F916}"} IA dédiée</button>
            <button onClick={() => { goPage("cert"); setCertProjet(p.nom); }} style={{ background: "rgba(82,195,122,0.06)", border: "0.5px solid rgba(82,195,122,0.2)", borderRadius: 10, padding: "7px 4px", fontSize: 9, fontWeight: 700, color: "#52C37A", cursor: "pointer" }}>{"\u{1F3C5}"} Certificat</button>
            <button onClick={() => genererCRChantier(p)} disabled={crLoading} style={{ background: "rgba(82,144,224,0.06)", border: "0.5px solid rgba(82,144,224,0.2)", borderRadius: 10, padding: "7px 4px", fontSize: 9, fontWeight: 700, color: "#5290E0", cursor: "pointer", opacity: crLoading ? 0.5 : 1 }}>{crLoading ? "..." : "\u{1F4CB} CR PDF"}</button>
          </div>
        </div>)}
      </div>
    </div>
